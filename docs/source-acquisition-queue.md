...
| 14 | P0 | governance_gap | SECNAV Instruction coverage gap | Department of the Navy Issuances | blocked_by_host | 96 | Use Department of the Navy Issuances to discover and register the first SECNAV Instruction artifact. |
| 15 | P1 | governance_gap | CISA Guidance / Model coverage gap | CISA | direct_fetch_ready | 94 | Use CISA to discover and register the first CISA Guidance / Model artifact. |
| 16 | P1 | source_start | Congress.gov initial ingest | Congress.gov | api_key_required | 93 | Configure API access or fallback search capture, then seed first source-known artifacts. |
| 17 | P1 | source_start | CISA initial ingest | CISA | direct_fetch_ready | 92 | Probe source discovery surface and seed the first official artifacts. |
| 18 | P1 | source_start | eCFR initial ingest | eCFR | direct_fetch_ready | 92 | Probe source discovery surface and seed the first official artifacts. |
| 19 | P1 | governance_gap | ALNAV coverage gap | MyNavyHR | blocked_by_host | 90 | Use MyNavyHR to discover and register the first ALNAV artifact. |
| 20 | P1 | coverage_completion | DoW/DoD Directive completion | DoW/DoD Issuances | blocked_by_host | 88 | Recover blocked raw source files, then regenerate extraction, claims, references, authority chain, and structured summary. |
| 21 | P1 | coverage_completion | DoW/DoD Instruction completion | DoW/DoD Issuances | blocked_by_host | 88 | Recover blocked raw source files, then regenerate extraction, claims, references, authority chain, and structured summary. |
